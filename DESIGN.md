...
**The Redundant Cue Rule.** Color accompanies names, icons, labels, strokes, and state attributes. Project logos identify the architecture choices alongside their names; color alone never carries the distinction.
...
- **Body:** reading and reference explanations use 0.875rem with generous 1.7 line height.
...
The learning pane uses compact headings so its steps and navigation controls fit without scrolling at the primary desktop size. Reference summaries are limited to 46ch and longer timeline explanations to 68ch. Numbered steps and zoom output use tabular numerals. Mobile and large-text styles alter the scale for their layouts.
...
At 760px and below, the page becomes stage-first, followed by the compact lesson steps. The header and architecture choices wrap, and an expandable All components list provides another route into the diagram. The mobile stage is 650px tall, or 770px while that list is open. Inspector details become a fixed bottom sheet. Reference layouts stack into one column; the wide comparison table keeps a contained horizontal scroller.
...
The main navigation uses text links with a blue lower border and stronger weight for the current page. The diagram toolbar's architecture picker shows Landscape and four project choices. Each project choice combines its name with an upstream project or publisher icon and a pressed state. Its trigger can collapse the choice row without changing the current diagram. Preserve real links for routes and buttons for in-page state changes.

### Evidence tags

Evidence tags use neutral fills, small semibold text, and explicit status names. Tags communicate source status rather than acting as colorful category decorations. Safety comparisons remain in the comparison view.
...
Model and policy nodes retain their existing violet and warm material treatments. Landscape project nodes show the matching project identifiers. In large-text mode, each node exposes outgoing destinations in text.
